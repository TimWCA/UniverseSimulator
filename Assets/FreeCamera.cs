...
public class FreeCamera : MonoBehaviour
{
    ...
    public void SetBoxColliderSize()
    {
        ...
        Vector3 point_B = mainCamera.ScreenPointToRay(new Vector2(UnityEngine.Screen.width, 0)).origin;
        ...
        point_B = mainCamera.ScreenPointToRay(new Vector2(0, UnityEngine.Screen.height)).origin;
    ...
    void Update()
    {
        ...
        if (Input.GetKey("escape"))  // если нажата клавиша Esc (Escape)
            if (MessageBox.Show("Вы уверены, что хотите выйти?", "Выход", MessageBoxButtons.YesNo) == DialogResult.Yes)
                UnityEngine.Application.Quit();    // закрыть приложение
    }
